...
if ( ! function_exists( 'trade_fair_get_loaded_wpemerge' ) ) {
	/**
	 * Get the currently loaded WP Emerge version, if any.
	 *
	 * @return array|false
	 */
	function trade_fair_get_loaded_wpemerge() {
		/**
...
if ( ! function_exists( 'trade_fair_should_load_wpemerge' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function trade_fair_should_load_wpemerge( $name, $min, $max ) {
		$loaded = trade_fair_get_loaded_wpemerge();
		...
		add_action(
			...
			function () use ( $name, $min, $max, $loaded ) {
				...
				$message = __( '%1$s requires WP Emerge version >= %2$s and < %3$s but %4$s has already loaded version %5$s.', 'trade_fair' );
				?>
...
if ( ! function_exists( 'trade_fair_declare_loaded_wpemerge' ) ) {
	/**
	 ...
	 * @return void
	 */
	function trade_fair_declare_loaded_wpemerge( $name, $type, $file ) {
		$loaded = trade_fair_get_loaded_wpemerge();
